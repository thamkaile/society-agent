...
const Threads = ({ color = [1, 1, 1], amplitude = 1, distance = 0, enableMouseInteraction = false, lineCount = 24, ...rest }) => {
  const containerRef = useRef(null);
  const animationFrameId = useRef(0);

  const propsRef = useRef({ color, amplitude, distance, enableMouseInteraction, lineCount });
  propsRef.current = { color, amplitude, distance, enableMouseInteraction, lineCount };
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      vertex: vertexShader,
      fragment: buildFragmentShader(propsRef.current.lineCount),
      uniforms: {
        iTime: { value: 0 },
        iResolution: {
